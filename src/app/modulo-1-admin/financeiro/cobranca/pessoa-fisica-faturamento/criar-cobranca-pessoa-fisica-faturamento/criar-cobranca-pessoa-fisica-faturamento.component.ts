...
@Component({
  selector: 'app-criar-cobranca-pessoa-fisica-faturamento',
  templateUrl: './criar-cobranca-pessoa-fisica-faturamento.component.html',
  styleUrls: ['./criar-cobranca-pessoa-fisica-faturamento.component.css']
})
export class CriarCobrancaPessoaFisicaFaturamentoComponent {
    ...
    observacoes : new FormControl('', [Validators.required]),
      ...
      onSubmit(): void {
        if (this.formCobranca.valid) {
          ...
          this.httpClient
            .post('http://localhost:8082/api/cobranca/cobrança-faturamento-pessoa-fisica', this.formCobranca.value)
            .subscribe({
              next: (data: any) => {
